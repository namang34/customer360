...
children.push(table([3100, 6260], [
  ...
  ["Escalation for ambiguity rather than cost (§6.3): should low confidence, or an unresolved disagreement, "
   ...
   "Not yet, and this is the clearest gap. The confidence gate turns low confidence into no_action, which is "
   + "auto-approved and never reaches a reviewer — right for the graded field, but it means the system is "
   + "silent exactly where it is least sure. The intended fix before the end-term submission is a review "
   + "queue for checkpoints where confidence is low but corroboration is broad."],
...
const diagram = [
  ...plate(
    ...
    "architecture.png", 720, 575, false),
  ...plate(
